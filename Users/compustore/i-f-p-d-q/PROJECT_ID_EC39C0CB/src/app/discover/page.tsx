...
export default function DiscoverPage() {
    ...
    const handleSearch = async () => {
        if (!usersIndex || !userProfile) {
            toast({ variant: 'destructive', title: 'Erreur', description: "Impossible d'initier la recherche."});
            return;
        }
        setIsSearching(true);
        ...
        try {
            ...
            toast({
                ...
                description: `${searchResults.length} profil(s) trouvé(s). Affichage des résultats.`,
            })
            router.push('/');
        } catch (error) {
